...
public class MapVisualizer {
    ...
    private String drawObject(Vector2d currentPosition) {
        Collection<Animal> animalsAtPosition = this.map.getAnimalsAt(currentPosition);
        Grass grassAtPosition = this.map.getGrassAt(currentPosition);

        if (!animalsAtPosition.isEmpty()) {
            if (animalsAtPosition.size() > 1) {
                return String.valueOf(animalsAtPosition.size());
            } else {
                return animalsAtPosition.iterator().next().toString();
            }
        } else if (grassAtPosition != null) {
            return grassAtPosition.toString();
        } else {
            return EMPTY_CELL;
        }
    }
